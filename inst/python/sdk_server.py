...
try:
    ...
    # Log where SDK is loaded from
    with open("/tmp/claude_debug.log", "a") as f:
        f.write(f"SDK loaded from: {query_module.__file__}\n")
    ...
    async def event_generator():
        ...
        async def run_query():
            try:
                ...
                # Make sure PATH includes common Claude CLI locations
                npm_global_bin = str(Path.home() / ".npm-global/bin")
                current_path = os.environ.get("PATH", "")
                if npm_global_bin not in current_path:
                    os.environ["PATH"] = f"{npm_global_bin}:{current_path}"
